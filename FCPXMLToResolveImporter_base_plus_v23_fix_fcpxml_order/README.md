...
修复“完整素材顺序时间线按文件名排序，而不是按 FCPXML 时间线顺序”的问题。

原因：
旧版在没有正确识别到时间线片段时，会退回到：
...
1. 先解析 FCPXML 中 `sequence/spine` 下的时间线片段；
2. 识别 `asset-clip / video / audio / clip / sync-clip / mc-clip / ref-clip` 的 `ref`；
3. 使用 `offset` 作为时间线位置排序；
4. 如果没有 `offset`，使用 XML 出现顺序；
5. 只有完全找不到时间线片段时，才退回到 asset 定义顺序，不再按文件名排序。
